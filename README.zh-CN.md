...
目前完整应用支持 **macOS**。Windows 是后续扩展方向，尚未实现和验证完整的 Windows 打印后端。文档不上传到服务器，应用不依赖云端转换服务。
...
XLSX 采用已用区域、缩放和纵向分页模型，**不等同于完整 Excel 打印引擎**，不保证还原打印区域、重复标题及全部页面布局功能。图片缺少可靠物理尺寸信息时，实际大小按 96 DPI 计算。Markdown 不下载远程图片，可以读取本地相对路径图片。
